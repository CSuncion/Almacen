...
{
    public partial class wDetalleOrdenCompra : Heredados.MiForm8
    {
        public wDetalleOrdenCompra()
        {
            InitializeComponent();
        }
        ...
        public void InicializaVentana()
        {
            ...
            if (wEditOrdCom.txtCodMonSyD.Text == "0")  //Soles
            {
        ...
        public void MostrarMovimientoDeta(MovimientoOCDetaEN pObj)
        {
            ...
            this.txtCodMon.Text = this.wEditOrdCom.txtCodMonSyD.Text.ToString().Trim();
        ...
        public void CargarPresupuesto()
        {
            if (txtCodAre.Text.Trim() != string.Empty)
            {
                ...
                this.txtPresupuesto.Text = this.eLisPre.Where(x => x.CodigoPresupuesto == wEditOrdCom.wOrdCom.lblPeriodo.Text
                && x.CCentroCosto == txtCodAre.Text.Trim()).Count() == 0 ? Formato.NumeroDecimal(0, 2) :
                Formato.NumeroDecimal(this.eLisPre.Where(x => x.CodigoPresupuesto == wEditOrdCom.wOrdCom.lblPeriodo.Text && x.CCentroCosto == txtCodAre.Text.Trim()).FirstOrDefault().SaldoPresupuesto.ToString(), 2);

                this.txtSaldoPresupuesto.Text = this.eLisPre.Where(x => x.CodigoPresupuesto == wEditOrdCom.wOrdCom.lblPeriodo.Text
                && x.CCentroCosto == txtCodAre.Text.Trim()).Count() == 0 ? Formato.NumeroDecimal(0, 2) :
                Formato.NumeroDecimal(this.eLisPre.Where(x => x.CodigoPresupuesto == wEditOrdCom.wOrdCom.lblPeriodo.Text && x.CCentroCosto == txtCodAre.Text.Trim()).FirstOrDefault().SaldoPresupuesto.ToString(), 2);
            }
        }
        ...
        public void RetornarPresupuesto()
        {
            ...
            string presupuesto = this.eLisPre.Where(x => x.CodigoPresupuesto == wEditOrdCom.wOrdCom.lblPeriodo.Text
                && x.CCentroCosto == txtCodAre.Text.Trim()).Count() == 0 ? Formato.NumeroDecimal(0, 2) :
                Formato.NumeroDecimal(this.eLisPre.Where(x => x.CodigoPresupuesto == wEditOrdCom.wOrdCom.lblPeriodo.Text && x.CCentroCosto == txtCodAre.Text.Trim()).FirstOrDefault().ImportePresupuesto.ToString(), 2);

            string nuevoPresupuesto = this.eLisPre.Where(x => x.CodigoPresupuesto == wEditOrdCom.wOrdCom.lblPeriodo.Text
                && x.CCentroCosto == txtCodAre.Text.Trim()).Count() == 0 ? Formato.NumeroDecimal(0, 2) :
                Formato.NumeroDecimal(this.eLisPre.Where(x => x.CodigoPresupuesto == wEditOrdCom.wOrdCom.lblPeriodo.Text && x.CCentroCosto == txtCodAre.Text.Trim()).FirstOrDefault().SaldoPresupuesto.ToString(), 2);
        ...
        private void wDetalleOrdenCompra_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void txtPreUni_Validated(object sender, EventArgs e)
        {
            ...
            {
                if (this.wEditOrdCom.txtTipCam.Text == "0.0000")
                {
                    Mensaje.OperacionDenegada("Debe ingresar un tipo de cambio", this.wEditOrdCom.wOrdCom.eTitulo);
                ...
                else
                {
                    ...
                    tc = Convert.ToDecimal(this.wEditOrdCom.txtTipCam.Text);
                ...
                case Universal.Opera.Modificar:
                    {
                        ...
                        if (this.eMovDet.CodigoCentroCosto != eCenCos)
                        {
                            this.CargarPresupuesto();

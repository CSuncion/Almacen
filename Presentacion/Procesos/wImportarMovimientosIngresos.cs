...
{
    public partial class wImportarMovimientosIngresos : Heredados.MiForm8
    {
        public wImportarMovimientosIngresos()
        {
            InitializeComponent();
        }
        ...
        {
            txtPerMovCab.Text = wIng.lblDescripcionPeriodo.Text;
        }
        ...
        public List<DataGridViewColumn> ListarColumnasDgvMovDet()
        {
            ...
            iLisRes.Add(Dgv.NuevaColumnaTextCadena(MovimientoDetaEN.CodAux, "Proveedor", 100));
        ...
        public List<ErrorCeldaExcel> ListarErroresCeldasExcel()
        {
            ...
            for (int i = 2; i < 100000; i++)
            {
                ...
                //validar dato vacio
                iErr = new ErrorCeldaExcel();
                iErr = MiExcel.ObtenerErrorExcelPorDatoVacio("Orden Compra", "D" + i, iHoja);
                if (iErr.Celda != string.Empty) { iLisRes.Add(iErr); }

                //validar valor igual a
                iErr = new ErrorCeldaExcel();
                iErr = MiExcel.ObtenerErrorExcelPorComparacion("Periodo", this.wIng.lblPeriodo.Text, "F" + i, iHoja);
                if (iErr.Celda != string.Empty) { iLisRes.Add(iErr); }
                ...
                //validar dato vacio
                iErr = new ErrorCeldaExcel();
                iErr = MiExcel.ObtenerErrorExcelPorDatoVacio("Ruc Proveedor", "L" + i, iHoja);
                if (iErr.Celda != string.Empty) { iLisRes.Add(iErr); }
